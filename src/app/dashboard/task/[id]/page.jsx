...
export default function TaskDetailPage() {
  ...
  // حالتان لتأمين توافق السيرفر مع المتصفح ومنع الـ Hydration Error
  ...
  const [task, setTask] = useState({
    ...
    description: "Setup api/register, api/login, and api/logout endpoints using Sanctum tokens.",
  ...
  const [comments, setComments] = useState([
    { id: 1, author: "Ali Mansour", content: "I migration tables are ready.", timestamp: "2026-08-13 14:30" },
    { id: 2, author: "Rama Ahmad", content: "Great work, testing now.", timestamp: "2026-08-13 15:00" },
  ]);
  const [newComment, setNewComment] = useState("");

  // قراءة الكوكيز بعد تحميل الصفحة بالكامل في المتصفح لمنع خطأ الـ Hydration
  useEffect(() => {
  ...
  const handleAddComment = (e) => {
    ...
    // تحديد اسم المعلق الوهمي حسب الحساب المفتوح حالياً في شريط التجربة
    const commentAuthor = userRole === "developer" ? "Ali Mansour" : userRole === "editor" ? "Rama Ahmad" : "Admin User";
  ...
  // انتظر التحميل لمنع التعارض بين الخادم والعميل
  if (!isMounted) return <div className="p-4 text-slate-500">Loading Task Details...</div>;

  // [تعديل الصلاحيات الحصري]: مسموح فقط للأدمن والمحرر، ومقفل تماماً على أي مطور (Developer)
  const canUpdateStatus = userRole === "admin" || userRole === "editor";

  return (
    <div className="space-y-6 text-left" dir="ltr">
      {/* رأس الصفحة وعرض نوع الصلاحية الحالية للتأكد */}
      <div className="flex justify-between items-center border-b border-slate-100 pb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => router.push("/dashboard/task")} className="text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 px-4 py-2 rounded-xl transition">
            ← Back
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* العمود الأساسي: تفاصيل الوصف والمطورين */}
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm space-y-2">
            ...
            <p className="text-sm text-slate-600 leading-relaxed bg-slate-50 p-3 rounded-lg border border-slate-100">{task.description}</p>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {task.assignees.map((dev) => (
                <div key={dev.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                  <div className="w-8 h-8 rounded-full bg-indigo-600 text-white flex items-center justify-center font-bold text-xs flex-shrink-0 select-none">{dev.name.charAt(0)}</div>
                  <div>
                    <h5 className="text-sm font-semibold text-slate-900">{dev.name}</h5>
        ...
        {/* العمود الجانبي: التحكم بالخيارات والتواريخ والحالة */}
        <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm space-y-4 h-fit">
          <div>
            ...
            <span className="text-sm font-medium text-slate-800">{task.due_date}</span>
          </div>
          <div>
            <span className="text-xs font-semibold text-slate-400 block uppercase mb-1">Priority</span>
            <span className="text-xs font-bold px-2.5 py-1 rounded-md bg-rose-50 text-rose-700 capitalize">{task.priority}</span>
          </div>
          
          {/* حقل الحالة التفاعلي المحمي بالصلاحيات */}
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase mb-1">Status</label>
            <select
              disabled={!canUpdateStatus} // ينقفل تلقائياً إذا كان المستخدم الحالي Developer
            ...
            {!canUpdateStatus && (
              <p className="text-[10px] text-rose-500 mt-1">Status can only be updated by Admin or Editor.</p>
            )}
          </div>
        </div>
      </div>

      {/* قسم التعليقات الكامل (Comments Section) */}
      <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm space-y-4">
        <h4 className="text-sm font-bold text-slate-800 border-b border-slate-100 pb-2">Comments & Discussion</h4>
        
        {/* قائمة عرض التعليقات المضافة مع الـ Avatar المحدث بـ Tailwind */}
        <div className="space-y-4 max-h-60 overflow-y-auto pr-2">
          {comments.map((c) => (
            <div key={c.id} className="flex gap-3 items-start p-3 bg-slate-50/70 rounded-xl border border-slate-100/80">
              {/* الـ Avatar الدائري المخصص للكاتب */}
